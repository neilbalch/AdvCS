...
//    }
    ...
    private boolean isBalanced(TNode<T> root) {
...
//    }
    private LinkedList<T> inOrderList(TNode<T> root) {
        LinkedList<T> list = new LinkedList<>();
        if (root == null) return null;

        LinkedList<T> tmp;
        tmp = inOrderList(root.getLeft());
        if (tmp != null) for (T item : tmp) list.add(item);
        list.add(root.get());
        tmp = inOrderList(root.getRight());
        if (tmp != null) for (T item : tmp) list.add(item);
    ...
    public void balance() {
        LinkedList<T> list = inOrderList(root);
        root = null;
        balancedAdd(list, 0, list.size() - 1);
    }

    private void balancedAdd(LinkedList<T> list, int start, int end) {
        if (start > end) return;
        int middlePos = (start + end) / 2;

        if (middlePos >= 0 && middlePos < list.size() && list.get(middlePos) != null) {
            add(list.get(middlePos));
            balancedAdd(list, start, middlePos - 1);
            balancedAdd(list, middlePos + 1, end);
        }
    }
